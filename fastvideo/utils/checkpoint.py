...
def save_final_model(transformer, rank, final_model_dir):
    """Save the final trained model to the specified directory.
    
    This saves the model directly to final_model_dir (not in a subdirectory),
    making it ready for direct loading with from_pretrained().
    """
    main_print(f"--> saving final model to {final_model_dir}")
    with FSDP.state_dict_type(
            transformer,
            StateDictType.FULL_STATE_DICT,
            FullStateDictConfig(offload_to_cpu=True, rank0_only=True),
    ...
    if rank <= 0:
        ...
        # save using safetensors
        weight_path = os.path.join(final_model_dir,
                                   "diffusion_pytorch_model.safetensors")
        save_file(cpu_state, weight_path)
        config_dict = dict(transformer.config)
        if "dtype" in config_dict:
        ...
        with open(config_path, "w") as f:
            json.dump(config_dict, f, indent=4)
    main_print(f"--> final model saved to {final_model_dir}")
