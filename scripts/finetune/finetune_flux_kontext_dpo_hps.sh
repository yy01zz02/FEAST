#!/bin/bash
# Training script for Flux Kontext with HPSv2 Reward
...
mkdir -p images
mkdir -p data/outputs/kontext_dpo_hps
...
# ============================================================================
echo "Step 2: Starting Flux Kontext Online DPO training with HPSv2..."

torchrun --nproc_per_node=4 --master_port 19008 \
    ...
    --allow_tf32 \
    --cfg 0.0 \
    --output_dir data/outputs/kontext_dpo_hps \
    --final_model_dir data/outputs/kontext_dpo_hps_final \
    --h 1312 \
    --w 784 \
    --sampling_steps 16 \
    --eta 0.3 \
...
echo "Model saved to: data/outputs/kontext_dpo_hps_final"
